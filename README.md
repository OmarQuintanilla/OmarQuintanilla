## Hi there 👋
# ¡Hola! Soy [Escribe tu nombre aquí] 👋

> Soy un estudiante de ingeniería informática al cual le gusta aprender.

## Sobre mí
Soy un desarrollador de software en formación, enfocado en crear soluciones web escalables y trabajar con bases de datos funcionales, tanto relacionales como no relacionales.

Mis áreas de interés principales incluyen el desarrollo backend y la gestión de datos.

## 🛠️ Mi Stack de Tecnologías

Aquí están algunas de las herramientas y tecnologías con las que trabajo:

### Lenguajes de Programación
* JavaScript
* SQL

### Frameworks y Librerías
* Node.js
* Express

### Gestores de Bases de Datos
* PostgreSQL
* Oracle SQL

### Control de Versiones
* Git
* GitHub

### Otras Tecnologías
* LaTeX

## 📫 Encuéntrame en
* **LinkedIn:** [https://www.linkedin.com/in/omar-ernesto-quintanilla-aguilar/](https://www.linkedin.com/in/omar-ernesto-quintanilla-aguilar-817aa8394)
* **GitHub:** [https://github.com/](https://github.com/OmarQuintanilla)
* **Email:** 00081523@uca.edu.sv
